...
//
// 12 種身體原型 x 10 個強度稱號 = 120 款怪物。造型素材見 components/artAssets.ts 的 MONSTER_ART
// (assets/sprites/monsters/ai/{原型}_open.png)——這款一律用既有的 PNG,不畫程序化像素圖。
...
// id 命名規則 `${archetype}-${slot}`(slot 1~10):畫面只看 `-` 前面的原型去挑圖,
// 所以 slot 換再多,素材檔還是那 12 張。verify-lane-run 有一項在盯「每種造型都指得到檔案」。
...
// 10 個強度稱號槽位。稱號只影響名字,不影響造型——同一個原型的 10 款共用同一張圖。
...
// 造型素材是 assets/sprites/monsters/ai/boss_tierN_open.png,依階級由樸素到華麗遞進——
// 不再是全部關卡共用同一隻「關卡魔王」。
