 ...
 * 技能書副本通關給幾本。
 *
 * 固定 1 本,不隨關卡成長:技能書等級只有 5 級(`MAX_SKILL_BOOK_LEVEL`),
 * 給 2 本的話兩場就跳一級,「這一場換到一本」的份量會整個消失。
 * 而且技能書碰不到理想路線(它只放大元素與主動的效果幅度,見 laneRunSkills),
 * 所以給得快一點不會讓遊戲變簡單——會消失的只有「拿到的時候有感覺」。
 */
export const GRIMOIRE_BOOKS = 1;
