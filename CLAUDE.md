...
路上一排一排出現閘門(勇者 x2 / 裝備強化 / 勇者 x0.5 / 血量 -30 …)與敵人波次,
...
| 養成 | 裝備/技能/寵物一路疊 | **只有轉職**,每 5 關一次,只調整起跑數值 |
...
**鐵則:轉職只調整「起跑時的人數、武器階級、戰力與血量倍率」,絕對不放慢跑速、不減少陷阱。**
...
2. 改動 `game/` 內任何檔案 → 跑對應的驗證腳本
   (`npx tsx scripts/verify-lane-run.ts` 與 `npx tsx scripts/verify-lane-jobs.ts`,必須全部 PASS)
