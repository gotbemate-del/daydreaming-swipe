...
**左右滑動的跑道闖關。** 角色自動往前跑,畫面分三條跑道,左右滑各切換一條。路上一排一排出現
閘門(攻擊 x2 / 血量 -30 / 攻擊 +12 …)與敵人,**只有你所在那條跑道的節點會生效**。
核心決策是「下一排我要站哪一條」,而且只有一兩秒可以決定。
...
|---|---|---|
| 核心賣點 | 不玩也在成長 | 玩得好才打得贏 |
| 玩家做什麼 | 什麼都不用做,開著就好 | 每 1~2 秒選一次跑道 |
...
- [x] 跑道畫面 + 滑動/按鈕/方向鍵三種操作(實機 16 項通過)
- [ ] 把裝備/技能接成「起跑數值加成」
